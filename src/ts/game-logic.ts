...
export class GameLogic {
  ...
  appendComputerClick(number: string): void {
    this.compClicks.push(number);
    this.allMadeClicks.push(number);
  }

  computerClickSpeed(): number {
  ...
  showWinLine(position: string) {
    const winLine = document.querySelector(".winLine") as HTMLDivElement;
  ...
  checkWin(): void {
    ...
    if (this.allMadeClicks.length === 9 && !this.endGame) {
      const numberContainer = $(".red-color") as HTMLSpanElement;
      setTimeout(() => {
        this.gameInDraw++;
    ...
    if (this.endGame) {
      const playerWinsContainer = $(
        ".playerName .yellow-color"
      ) as HTMLSpanElement;
      const computerWinsContainer = $(
        ".computer .yellow-color"
      ) as HTMLSpanElement;
      ...
      const computerWinsNum = document.createTextNode(
        this.computerWins.toString()
      );
  ...
  boardClicks(): void {
    ...
    const handleClick = (e: MouseEvent) => {
      ...
      /* Show Comp Click */
      const writeCompClick = (number: string) => {
